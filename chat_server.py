...
class Server:
    def __init__(self):
        ...
        self.logged_name2sock = {}  # dictionary mapping username to 0
...
# ==============================================================================
                ...
                sender=msg["from"]
                text=msg["message"]
                full_message=sender+" "+text
                self.indices[from_name].add_msg_and_index(full_message)
                ...
                for g in the_guys:
                    ...
                    # IMPLEMENTATION
                    # ---- start your code ---- #
                    self.indices[g].add_msg_and_index(full_message)
                    mysend(
                        to_sock, json.dumps({"action": "exchange", "from": sender, "message": text}))
...
# ==============================================================================
                ...
                poem_data=self.sonnet.get_poem(poem_idx)
                if isinstance(poem_data,list):
                    poem="\n".join(poem_data)
                else:
                    poem=poem_data
...
# ==============================================================================
                ...
                # ---- start your code ---- #
                term=msg["target"]
                from_name=self.logegd_sock2name[from_sock]
                results=self.indices[from_name].search(term)
                if isinstance(results, list) and len(results) > 0:
                    # 将包含关键词的每一条聊天记录拼接成多行文本返回
                    search_rslt = '\n'.join([res[1] if isinstance(res, tuple) else str(res) for res in results])
                elif len(results) == 0:
                    search_rslt = f"No results found for '{term}'"
                else:
                    search_rslt = str(results)
